...
VERSIÓN CORREGIDA - BNO055 ahora recibe el bus I2C correctamente
...
# -----------------------------------------------------------------------------
# VERSIÓN CORREGIDA: Ahora acepta i2c_bus en el constructor
class BNO055Sensor(SensorBase):
    ...
    """
    def __init__(self, i2c_bus, address: int = 0x28):  # CORREGIDO: ahora acepta i2c_bus
        super().__init__("BNO055")
        self.i2c_bus = i2c_bus  # Guardamos el bus (aunque no lo usemos directamente)
        self.bus_id = 1  # El bus ID es siempre 1 en Raspberry Pi
        self.address = address
        self.sensor = None
        self._data = {
            # Aceleración
...
class SensorManager:
    ...
    def init_all(self) -> bool:
        ...
        # BNO055 - CORREGIDO: Ahora pasa self.i2c
        if self.use_bno055:
            for addr in [0x28, 0x29]:
                sensor = BNO055Sensor(self.i2c, address=addr)  # CORREGIDO: pasamos self.i2c
                if sensor.init():
